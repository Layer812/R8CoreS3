...
version 43
__lua__
function _init()cartdata"lootslime"nB()n3()end function n3()P=0nC()nD()n4()nE()nF()nG()nH()nI()ns()nJ()palt(0,false)palt(15,true)n6()nK()nL()end function _update()if(Q)nM()return
nN()if(j>0)j-=1return
nO()nP()nQ()nR()nS()nT()for n in all(u)do nU(n)end nV()nW()nX()nY()nZ()en()end function _draw()if(Q)ee()return
cls()ed()eo()local n,e=ef()camera(n7+n,n9+e)map()map(0,0,0,0,128,64,128)et()nb()for n in all(u)do e1(n)end el()map(0,0,0,0,128,64,127)ei()e0()ea()ec()er()e2()eh()eu()np()e8()e5(l.x-8,l.y)end n7=0n9=0i=0a=0function nR()local e,n=flr((l.x+2)/128),flr((l.y+4)/128)if e~=i or n~=a do if(n>3or n<0)return
i=e a=n n6()e3()nn=false e4()es()e6()e7()if(i==3and a==1and F>=50and not ne)ny()
if(l.x>=376and l.x<=384and l.y>=168and l.y<=216and ne)e9()
end i=e a=n n7=i*128n9=a*128end function k(n,e)return n.x+1<e.x+8and n.x+7>e.x and n.y+5<e.y+8and n.y+8>e.y end function e7()if a<4do for n=1,#g do local n=g[n]if(n.x==i and n.y==a)n.visited=true eb()break
end end end function nC()nd=0q=0z=0R=0end function nN()nd+=1if(nd>=30)nd-=30q+=1dset(6,q)
if(q>=60)q-=60z+=1dset(7,z)
if(z>=60)z-=60R+=1dset(8,R)
end function nI()l={sprite=1,x=181,y=184,prev_x=0,prev_y=0,vely=0,prev_vely=0,velx=0,prev_velx=0,hit_bottom=false,hit_left=false,hit_right=false,hit_top=false,grounded=false,facing="right",jumpting=false,falling=true,wall_jump_timer_left=0,wall_jump_timer_right=0,jumps=0,outline=false,last_safe={x=0,y=0},on_ladder=false,dead=false,coyote=3}end function nS()if(l.dead)return
if not l.grounded and l.coyote>0do l.coyote-=1elseif l.grounded and not l.hit_left and not l.hit_right do l.coyote=3end l.prev_vely=l.vely if(l.wall_jump_timer_left>0or l.wall_jump_timer_right>0)l.wall_jump_timer_left-=1l.wall_jump_timer_right-=1
ep()if(l.hit_left or l.hit_right)and l.vely>0do l.vely+=.1if(l.vely>1.5)l.vely=1.5
else l.vely+=.3end if(l.on_ladder)l.vely=0
ey()em()if l.velx<.4and l.velx>-.4do l.velx=0elseif l.velx<0do if(l.grounded)l.velx+=.3else l.velx+=.2
elseif l.velx>0do if(l.grounded)l.velx-=.3else l.velx-=.2
end if(l.prev_vely-l.vely>2)ek()
if(abs(l.velx)>1.5)l.velx=sgn(l.velx)*1.5
l.prev_x=l.x l.prev_y=l.y eg()end function ew(n)local e=max(1,ceil(abs(n)))local n=n/e for e=1,e do if not l.grounded do l.y+=n elseif l.vely<0do l.y+=n end no()end end function ev()if(fget(mget(l.x/8,l.y/8),1))return false
local n,e=l.x,l.y+8local d,n=mget((n+1)/8,e/8),mget((n-1)/8,e/8)if(not fget(d,2)and not fget(n,2)and fget(d,0)and fget(n,0)and l.grounded)return true
return false end function em()if(ev())l.last_safe.x=l.x l.last_safe.y=l.y dset(20,l.last_safe.x/8)dset(21,l.last_safe.y/8)
ew(l.vely)no()if(l.grounded and l.hit_top)if(mget(l.x/8,(l.y-1)/8)==0)l.y-=1else l.x+=1l.vely=0
if l.grounded and l.vely>0do l.y-=(l.y+8)%8l.vely=0elseif l.hit_top and l.vely<0do l.y+=(4-l.y%8)%8end e_()end function e_()l.x+=l.velx no()if l.hit_left do local n=flr(l.x/8)l.x=n*8+7l.velx=0l.sprite=1elseif l.hit_right do local n=flr((l.x+8)/8)l.x=n*8-6if(l.velx>0)l.velx=0
l.sprite=2end end function nb()if(l.dead)return
ej()if(not l.on_ladder)spr(l.sprite,l.x,l.y)
end function ep()if l.velx<0do l.facing="left"elseif l.velx>0do l.facing="right"end end function ey()if(l.on_ladder)ex()return
if(btn(0)and l.wall_jump_timer_left<=0)l.sprite=2l.velx-=1w=0
if(btn(1)and l.wall_jump_timer_right<=0)l.sprite=1l.velx+=1w=0
if(btnp(3)and nf)r=true nm()
if(btnp(5)and nt==true and b.f>6)nm()
if(btnp(4))w=0if l.grounded or l.coyote>0do l.grounded=false l.vely=-3sfx(0)l.coyote=0elseif l.hit_left and S do l.vely=-3l.velx=3l.wall_jump_timer_left=16l.wall_jump_timer_right=0sfx(0)elseif l.hit_right and S do l.vely=-3l.velx=-3l.wall_jump_timer_right=16l.wall_jump_timer_left=0sfx(0)elseif l.jumps>0do l.vely=-2.5l.jumps=0sfx(0)end
if(l.jumps==0)l.outline=false
if(l.grounded)l.outline=false l.jumps=0
if(btn(2)or btn(3))local n,e=flr(l.x/8),flr(l.y/8)if(mget(n,e)==103or mget(n+1,e)==103or mget(n,e+1)==103or mget(n+1,e+1)==103)l.on_ladder=true l.velx=0l.vely=0
end function no()local n,e=l.x,l.y l.grounded=p(n+4,e+8,0)or p(n+2,e+8,0)or p(n+4,e+8,3)or p(n+2,e+8,3)l.hit_top=p(n+4,e+4,0)or p(n+1,e+4,0)l.hit_left=p(n,e+4,0)l.hit_right=p(n+7,e+4,0)end function p(n,e,d)eq=mget(flr(n/8),flr(e/8))return fget(eq,d)end function eg()local n,e=l.x+3,l.y+5if(l.y>512)n1()return
local d,n,o,e=n-.5,n+.5,e-.5,e+.5if(fget(mget(flr(d/8),flr(o/8)),1)or fget(mget(flr(n/8),flr(o/8)),1)or fget(mget(flr(d/8),flr(e/8)),1)or fget(mget(flr(n/8),flr(e/8)),1))n1()
end G={}function n1()sfx(9)if(not Q)P+=1
dset(5,P)l.dead=true l.velx=0l.vely=0G={}for n=0,35do local n=n/8add(G,{x=l.x+3,y=l.y+4,vx=cos(n)*.5,vy=sin(n)*.5,t=0})end end function np()if(not l.dead)return
for n in all(G)do n.x+=n.vx n.y+=n.vy n.t+=1if n.t<12do circfill(n.x,n.y,1,H)elseif n.t<18do circfill(n.x,n.y,.5,H)else G={}l.x=l.last_safe.x l.y=l.last_safe.y l.dead=false y={}end end end y={}ez=3I=0nl=6H=3function eA()local n,e=l.x+3-l.velx*.01,l.y+nl-l.vely*.01add(y,{x=n,y=e})if l.hit_left do I=-1elseif l.hit_right do I=1else I=0end while(#y>ez)del(y,y[1])
if(btn(2)and btnp(5))H=(H+1)%15
end function ej()if(l.grounded)nl=6else nl=6
eA()for e=1,#y-1do local n,d=y[e],y[e+1]for o=0,1,.2do local f,n,e=n.x+(d.x-n.x)*o,n.y+(d.y-n.y)*o,e if(l.outline)circfill(f+I,n,e+1,7)
circfill(f+I,n,e,H)end end end x={}function ek()for n=1,10do add(x,{x=l.x+3,y=l.y+8,spdx=rnd(1)-.5,spdy=rnd(.5)-.5,radius=rnd(3),life=rnd(20),colour=6})if(rnd()<.5)x[n].colour=5
end end function nT()for n in all(x)do n.x+=n.spdx n.y+=n.spdy n.life-=1if(n.life<=3)n.radius=.5
if(n.life<=0)del(x,n)
end end function ea()for n in all(x)do circfill(n.x,n.y,n.radius,n.colour)end end v={}function nJ()for n in all(v)do del(v,n)end for n=1,100do add(v,{x=flr(rnd(128)),y=flr(rnd(128)),spdx=rnd(.1)-.05,spdy=rnd(.1)-.05,colour=1})local e=flr(rnd(4))if e==1do v[n].colour=5elseif e==2do v[n].colour=6end end end function nQ()for n in all(v)do n.x+=n.spdx n.y+=n.spdy n.x=(n.x+135)%135n.y=(n.y+135)%135end end function ed()for n in all(v)do pset(n.x+i*128,n.y+a*128,n.colour)end end T={}function e6()for n in all(T)do mset(n.x,n.y,98)end T={}for e=0,15do for n=0,15do local n,e=i*16+n,a*16+e if(mget(n,e)==98)add(T,{x=n,y=e,f=0,fading=false})
end end end function nZ()for n in all(T)do local e,d=n.x*8,n.y*8if(k({x=l.x,y=l.y+1},{x=e,y=d})or k({x=l.x-1,y=l.y-2},{x=e,y=d}))n.fading=true
if(n.fading and n.f==0)sfx(6)
if(n.fading)n.f+=.2
if(n.f>=0)if(n.f<4)mset(n.x,n.y,98+flr(n.f))else mset(n.x,n.y,0)
if(n.f>20)n.f=0n.fading=false
end end m={}function es()for n in all(m)do mset(n.x,n.y,n.c=="red"and 92or 108)end m={}for e=0,15do for n=0,15do local n,e=i*16+n,a*16+e local d=mget(n,e)if d==92do add(m,{x=n,y=e,c="red",timer=0})mset(n,e,0)elseif d==108do add(m,{x=n,y=e,c="blue",timer=0})mset(n,e,0)end end end end function nY()if(#m>1and(m[1].timer==0or m[1].timer==30))sfx(6)
for n in all(m)do if n.c=="red"and n.timer<30do mset(n.x,n.y,92)elseif n.c=="blue"and n.timer>30do mset(n.x,n.y,108)else mset(n.x,n.y,0)end n.timer=(n.timer+1)%60end end function ex()w=0if(l.grounded)l.on_ladder=false
local n,e=flr(l.x/8),flr(l.y/8)if(not(mget(n,e)==103or mget(n+1,e)==103or mget(n,e+1)==103or mget(n+1,e+1)==103))l.on_ladder=false
if btn(0)do if(l.hit_top)l.y+=1
l.velx-=.5end if btn(1)do if(l.hit_top)l.y+=1
l.velx+=.5end if(btn(2)and not l.hit_top)l.y-=1
if(btn(3)and not l.grounded)l.y+=1
if(btnp(4))sfx(0)l.on_ladder=false l.vely=-3
end u={seg={}}function n6()u={}local n,e=i*16,a*16for d=0,15do for o=0,15do local n,e=n+o,e+d local d=mget(n,e)if d==8do add(u,J(n*8+1,e*8,5,4))add(u,J(n*8+3,e*8,5,7))add(u,J(n*8+6,e*8,5,2))elseif d==9do add(u,J(n*8+4,e*8,6,4))add(u,J(n*8+6,e*8,5,3))end end end end function J(e,d,n,f)local o={seg={},len=n,falling=false}for f=1,f do add(o.seg,{x=e,y=d+(f-1)*n,px=e,py=d+(f-1)*n})end return o end function nU(n)if(n.falling)for n in all(n.seg)do local e,d=(n.x-n.px)*.95,(n.y-n.py)*.95n.px=n.x n.py=n.y n.x+=e n.y+=d+.2end return
for e=2,#n.seg do local n=n.seg[e]local e,d=(n.x-n.px)*.95,(n.y-n.py)*.95n.px=n.x n.py=n.y n.x+=e n.y+=d+.1end for e=1,3do for t=2,#n.seg do local e,d=n.seg[t-1],n.seg[t]local o,f=d.x-e.x,d.y-e.y local l=sqrt(o*o+f*f)local n=(l-n.len)/l if(t>2)e.x+=o*n*.5e.y+=f*n*.5
d.x-=o*n*.5d.y-=f*n*.5end end eB(n,l.x+4,l.y+4)end function eB(n,e,d)for o=2,#n.seg do local n=n.seg[o]local e,d=n.x-e,n.y-d local o=e*e+d*d if(o<8)n.x+=e*.3n.y+=d*.3
end end function e1(n)for e=2,#n.seg do local d,n=n.seg[e-1],n.seg[e]line(d.x,d.y,n.x,n.y,3)end end function nk(l,i,d)local o={}for n in all(u)do for e=#n.seg,2,-1do local f,t=n.seg[e-1],n.seg[e]local a,f=(f.x+t.x)/2,(f.y+t.y)/2if((a-l)^2+(f-i)^2<d*d)add(o,{v=n,i=e})break
end end for n in all(o)do eC(n.v,n.i)end end function eC(n,e)if(#n.seg-e+1<2)return
local d={seg={},len=n.len,falling=true}for e=e,#n.seg do add(d.seg,n.seg[e])end for e=#n.seg,e,-1do deli(n.seg,e)end add(u,d)end s={}A=1function ni(n,e,d)add(s,{x=n*8,y=e*8,f=0,collected=false,collectedf=0,kind=d or A})A=A%5+1end function nF()s={}for n=0,63do for e=0,127do local d=mget(e,n)if(d==64)ni(e,n)mset(e,n,0)
end end for n in all(c)do n.kind=A A=A%5+1end end function nV()if(F>=50and not n0)if(i==3and a==1)n0=true ny()else n0=true ng()
for n in all(s)do n.f=(n.f+.2)%4if k(l,n)do if not n.collected do sfx(1)n.collected=true U[n.kind]+=1F+=1eD()for e=1,#c do local e=c[e]if(n.x/8==e[3]and n.y/8==e[4])e.coin_found=true break
end eE()end end if(n.collected and n.collectedf<5)n.collectedf+=.4
end end eF={{14,2},{12,1},{9,4},{11,3}}function el()for n in all(s)do if n.collected do if(n.collectedf<5)spr(68+flr(n.collectedf),n.x,n.y)
else local e=eF[n.kind]if(e)pal(7,e[1])pal(6,e[2])
spr(64+flr(n.f),n.x,n.y)pal(7,7)pal(6,6)end end end function na(n)local d={}for n in all(split(n,";"))do local n,e=split(n,","),{}if(#n==2)e={x=tonum(n[1]),y=tonum(n[2])}else for d=1,#n do e[d]=tonum(n[d])end
add(d,e)end return d end eG="17,62,18,62;15,18,11,26;26,18,22,17;39,30,37,30;80,29,81,22;58,2,61,3;50,10,51,10;111,23,108,20;119,14,119,27;119,6,121,2;34,9,38,9;15,12,7,14;29,6,28,9;93,58,91,58;72,23,72,26;29,48,31,52;120,36,118,35;106,52,109,53;98,1,100,1;99,17,96,16;41,62,43,59"function nD()c=na(eG)for n in all(c)do n.found=false n.coin_found=false end end function nW()for n in all(c)do if(k(l,{x=n[1]*8,y=n[2]*8})and not n.found)sfx(5)mset(n[1],n[2],0)j=33ni(n[3],n[4],n.kind)n.found=true eH()
end end S=false nt=false nf=false nw={{x=72,y=27,sprx=10,spry=5,reward=function()S=true dset(0,1)end,found=false,message={"you found climbing boots!","now you can wall jump!"}},{x=97,y=30,sprx=7,spry=5,reward=function()nc=true nv()dset(1,1)end,found=false,message={"you found the map!","stand still to have a look","at it!"}},{x=124,y=18,sprx=11,spry=4,reward=function()nt=true dset(2,1)end,found=false,message={"you found the machete!","press ❎ to slash!"}},{x=8,y=33,sprx=10,spry=4,reward=function()nf=true dset(3,1)end,found=false,message={"you found the shovel!","press ⬇️ to dig!"}},{x=84,y=5,sprx=8,spry=5,reward=function()nr=true dset(4,1)end,found=false,message={"you found the compass!","it shows treasure on the map!"}}}K={active=false,message="press ⬆️ to open chest"}function nX()local e=false for n in all(nw)do if(n.found)mset(n.x,n.y,89)else mset(n.x,n.y,73)
if not n.found do if(k(l,{x=n.x*8,y=n.y*8}))K.active=true e=true
if(k(l,{x=n.x*8,y=n.y*8})and btnp(2))sfx(5)n.reward()n.found=true nn=true V=n.message o.showing=true o.x=n.sprx o.y=n.spry o.dx=n.x*8o.dy=n.y*8o.timer=0o.timer2=0
end end if(not e)K.active=false
end function eh()if(K.active)local e,d,o=#K.message*4,i*128+6,i*128+128-6local n=l.x-e/2n=mid(d,n,o-e)n2(K.message,n,l.y+10,7,3)
end j=0o={}function ei()if o.showing do local n=flr(o.timer)for e=0,n do local d,n=e,o.dy+8-1-(n-e)-6if(d<8)sspr(o.x*8,o.y*8+d,8,1,o.dx,n)
end if(o.timer<8)o.timer+=.2
o.timer2+=.2if(o.timer2>30)o.showing=false
end end function nE()U={0,0,0,0,0}F=0n0=false eI=na"51,20;57,22;51,29;55,27;60,29"n_=na"3,4;3,8;3,12;7,1;7,5;7,9;7,13;11,4;11,8;11,12"end eJ={16,18,20,22,16}eK={40,40,40,40,42}function e0()for n=1,5do local e=eI[n]for d=1,U[n]do sspr(eJ[n],eK[n],2,2,e.x*8+n_[d].x,e.y*8+n_[d].y)end end end b={f=5.9}B=3r=false function nm()b.f=0sfx(3)end function eL()local d,o=flr(l.x/8+B/4),flr(l.y/8)if(r)d=flr(l.x/8)o=flr(l.y/8)+1
for f=1,2do local n,e=d,o-1+f if(r)n=d-1+f e=o
local d=mget(n,e)if not r and(d==25or d==42)or r and d==10do mset(n,e,0)sfx(4)if r do for d=1,#C do local d=C[d]if(d.x==n and d.y==e)d.cleared=true eM()break
end end for d=1,20do local d=3if(rnd()<.5)d=11
if(r)d=4if(rnd()<.5)d=2
add(x,{x=n*8+4,y=e*8+4,spdx=rnd(1)-.5,spdy=rnd(1)-.5,radius=rnd(3),life=rnd(20),colour=d})end end end end function nP()if l.facing=="right"do B=3elseif l.facing=="left"do B=-3elseif r do B=0end if(not r and b.f<4)nk(l.x+B,l.y,10)nk(l.x+5,l.y,10)
if(b.f<3)eL()
if(b.f<6)b.f+=.4else r=false
end function ec()if(b.f<4)if(r)spr(76+b.f,l.x,l.y+4)else spr(83+b.f,l.x+B,l.y+2,1,1,l.facing=="left")
end nn=false V={"test"}function e2()if nn do local e,d,o,n,f,n,n=i*128+10,a*128+30,i*128+117,a*128+97,#V*6,l.y%128if(i==7and a==1or i==5and a==0)n=d+100else n=d+30
local f,t=n-f/2,sin(time()*.2)for n=1,#V do local d=V[n]local l=#d*4local e,n=e+(o-e)/2-l/2,f+(n-1)*6-30+t n2(d,e,n,7,3)end end end function n2(n,e,d,o,t)for o=-1,1do for f=-1,1do if(o~=0or f~=0)?n,e+o,d+f,t
end end?n,e,d,o
end function nj(n,e,d)return{x=n,y=e,spdx=rnd(1)-.5,spdy=rnd(1)-1,r=rnd(2),life=d or 0,colour=8+flr(rnd(3))}end function nx(e,d)for n in all(e)do n.x+=n.spdx/2n.y+=n.spdy n.life+=.2if(n.life>3.4)n.r=.5
if(n.life>4)del(e,n)
circfill(n.x,n.y+d,n.r,n.colour)end end function e3()nq={}local n,e=i*16,a*16for d=0,15do for o=0,15do local f=mget(n+o,e+d)if(f==47)add(nq,{x=(n+o)*8,y=(e+d)*8,f=rnd(1),active=true,timer=0,particles={},fade=false})
end end end function et()for n in all(nq)do spr(51,n.x,n.y)if not n.active do n.timer+=1if(n.timer>60)n.active=true
if(not n.fade)n.fade=true n.particles={}for e=1,10do add(n.particles,nj(n.x+4+rnd(2)-1,n.y+4+rnd(2)-1))end
nx(n.particles,0)else n.timer+=.01local e=sin(n.timer+n.f)*2spr(47,n.x,n.y+e)if(k(l,n))l.jumps=1n.active=false n.timer=0l.outline=true sfx(4)
while(#n.particles<15)add(n.particles,nj(n.x+4+rnd(2)-1,n.y+4+rnd(2)-1,rnd(3)))
nx(n.particles,e)end end end nc=false nr=false w=0W=0Y=true function nG()g={}for n=0,3do for e=0,7do add(g,{x=e,y=n,treasure=true,visited=false})end end end function nv()menuitem(2,"map "..(Y and"enabled"or"disabled"),function()Y=not Y nv()return true end)end function e4()for e in all(g)do if e.treasure do local n,d,o=false,e.x*16,e.y*16if not n do for e in all(c)do if(e[3]>=d and e[3]<d+16and e[4]>=o and e[4]<o+16and not e.coin_found)n=true break
end end if not n do for e in all(s)do if(e.x/8>=d and e.x/8<d+16and e.y/8>=o and e.y/8<o+16and not e.collected)n=true break
end end if(not n)e.treasure=false
end end end function en()if(w<65)w+=1
W=(W+1)%30end function er()if(not nc or not Y)return
if(w<60)return
local n,e=4+i*128,2+a*128rectfill(n,e,n+34,e+18,5)rectfill(n+1,e+1,n+33,e+17,0)for d in all(g)do if(d.visited)rectfill(2+n+d.x*4,2+e+d.y*4,4+n+d.x*4,4+e+d.y*4,5)if(W<15and nr and d.treasure)rectfill(2+n+d.x*4,2+e+d.y*4,4+n+d.x*4,4+e+d.y*4,6)
end if(W<15)rectfill(2+n+i*4,2+e+a*4,4+n+i*4,4+e+a*4,11)
end nh={}eN={["0,2"]=true,["0,3"]=true,["1,2"]=true,["1,3"]=true,["2,2"]=true,["2,3"]=true,["6,2"]=true}function eO()return eN[i..","..a]end function nK()nh={}for n=1,100do add(nh,{x=rnd(128),y=rnd(128),spdx=rnd(2)-1,spdy=rnd(1),vx=0,vy=0,r=rnd(2)})end end function eu()if(not eO())return
local d,o,e,f=i*128,a*128,(l.prev_x or l.x)+3,(l.prev_y or l.y)+5for n in all(nh)do local e,f=n.x-e,n.y-f local f=sqrt(e*e+f*f)if f<10do local d=(10-f)/10if(l.velx~=0and e*l.velx<0)n.vx+=l.velx*d*.6n.vy+=l.vely*d*.2
n.vy-=d*.2end n.vx*=.92n.vy*=.92if(e*l.velx>0)n.vx*=.8
n.vx=mid(-1.2,n.vx,1.2)n.x+=n.spdx+n.vx n.y+=n.spdy+n.vy n.x=d+(n.x-d)%128n.y=o+(n.y-o)%128circfill(n.x,n.y,n.r,7)end end nu={}function nL()nu={}for n=1,300do add(nu,{angle=rnd(1),dist=22+rnd(10),speed=.025+rnd(.05),col=6+rnd(2),r=.5+rnd(1)})end end function nz(f,d,l)for n in all(nu)do local e=n.angle+t()*n.speed local o,e=cos(e)*n.dist,sin(e)*n.dist*.35local f,e=f+o,d+e+o*.35if l do if(e>d)circfill(f,e,n.r,n.col)
else if(e<=d)circfill(f,e,n.r,n.col)
end end end function eo()local n=448+sin(t()/30)*2+sin(t()/25)*.5nz(576,n,false)circfill(576,n,15,1)circfill(574,n-2,14,0)nz(576,n,true)end Q=false n={x=-50,y=65,sprite=94,flip_x=false}function eP()spr(n.sprite,n.x,n.y,2,3,n.flip_x)end function e9()Q=true l.x=200l.y=80l.sprite=2camera(0,0)for n=0,15do for e=0,15do mset(e,n,0)end end for n=0,15do mset(n,11,58)end end f=0d=0function nM()cls()d+=1if(n.x%8>4)n.y=63else n.y=64
if f==0do if l.x>97do l.x-=1.5elseif n.x<27do n.x+=1end if(n.x==-30)sfx(7)
if(n.x==26and f==0)h()
end if f==1do if(d==70)sfx(8)e={x=38,y=78,particles={}}eQ()
if(d>70)e.x+=4e.y+=.25
if(e.x>97)e.x=-10e.y=-10h()n1()
end if(f==2)np()if(G[1].t==17)h()
if(f==3)if(d==30)music(0)h()
if f==4do?"+3 xp",X.x,X.y,3
if(X.y>68)X.y-=.2
if(d>120)h()
end if f==5do if(d==0)sfx(7)n.flip_x=true
n.x-=1if(d==180)h()
end if f==6do if(d==0)music(1)
if(d==224)h()
end local n="lootslime"if f==7do?n,64-#n*2,56,7
if(d==112)h()
end if f==8do?n,64-#n*2,56,7
local n="by patrick callaghan"?n,64-#n*2,64,5
if(d==112)h()
end if f==9do local n="thank you for playing!"?n,64-#n*2,56,7
if(d==224)h()
end if f==10do local n="deaths: "..P?n,64-#n*2,56,7
if(d==224)h()
end if f==11do function n8(n)return n<10and"0"..n or n end local n="time: "..n8(R)..":"..n8(z)..":"..n8(q)?n,64-#n*2,56,7
if(d>=224)local n="now see if you can do it faster!"?n,64-#n*2,64,7
end end e={x=-10,y=-10}X={x=88,y=76}function eQ()for n=0,50do add(e.particles,{x=e.x,y=e.y,spdx=rnd(1)-.5,spdy=rnd(2)-2,r=rnd(2),t=rnd(10)})end end function h()f+=1d=0end function ee()nb()eP()map()spr(91,e.x,e.y)for n in all(e.particles)do n.x+=n.spdx n.y+=n.spdy circfill(n.x,n.y,n.r,8+rnd(2))n.t+=1if e.x>0do if(n.t>6)n.x=e.x+rnd(7)n.y=e.y+rnd(6)n.t=0
else if(n.t>50)del(e.particles,n)
end end end Z={}D=0ne=false function eR()for n=0,100do add(Z,{x=384+rnd(8),y=168+rnd(64),r=rnd(3),c=5+rnd(2),spdy=rnd(),spdx=rnd()})end end function eS()for n=21,26do for e=48,49do mset(e,n,0)end end mset(48,20,53)mset(48,27,21)end function e8()if(#Z==0)return
if D==2do D-=1eS()elseif D>0do D-=1end for n in all(Z)do if D<2do n.r-=rnd(.2)if(n.r<0)del(Z,n)
else n.r=(n.r+rnd(.5))%3end circfill(n.x,n.y,n.r,n.c)end end function ny()eR()D=115ng()ne=true end function ng()eT(4,115)sfx(2)end L=0E=0function eT(n,e)E=n L=e end function nO()if(L>0)L-=1if(L<=0)E=0
end function ef()if(L>0)return rnd(E*2)-E,rnd(E*2)-E
return 0,0end M="secret!"nA=0_=0function e5(e,d)if(j<=0and _<=0)return
nA+=.1local n=#M*4e=mid(i*128+4,e,i*128+128-n-4)d=mid(a*128+4,d,a*128+128-6-4)if(j>0)_=min(_+.5,#M)else _=max(_-.5,0)
local n,o=1,flr(_)if(j<=0)n=#M-flr(_)+1o=#M
for n=n,o do local o,e,n=sub(M,n,n),e+(n-1)*4,d+sin(nA/1.5+n*.8)*2n2(o,e,n,7,13)end end function ns()S=dget(0)~=0nc=dget(1)~=0nt=dget(2)~=0nf=dget(3)~=0nr=dget(4)~=0P=dget(5)q=dget(6)z=dget(7)R=dget(8)eU()eV()eW()eX()eY()eZ()l.last_safe.x=dget(20)*8l.last_safe.y=dget(21)*8if(not(dget(20)==0and dget(21)==0))l.x=l.last_safe.x l.y=l.last_safe.y
end function nH()C={}for n=0,63do for e=0,127do if(mget(e,n)==10)add(C,{x=e,y=n,cleared=false})
end end end function eM()N(C,"cleared",22,23)end function eY()O(C,"cleared",22,23)for n in all(C)do if(n.cleared)mset(n.x,n.y,0)
end end function eU()for n=1,5do nw[n].found=dget(n-1)~=0end end function N(e,f,n,t)local d,o=0,0for n=1,#e do if(e[n][f])if(n<=16)d|=1<<n-1else o|=1<<n-17
end dset(n,d)dset(t,o)end function O(e,d,n,o)local o,f=dget(n),dget(o)for n=1,#e do if(n<=16)e[n][d]=o&1<<n-1~=0else e[n][d]=f&1<<n-17~=0
end end function eH()N(c,"found",10,13)end function eV()O(c,"found",10,13)for n in all(c)do if(n.found)mset(n[1],n[2],0)
end end function eE()N(c,"coin_found",11,14)end function eX()O(c,"coin_found",11,14)for n in all(c)do if(n.found and not n.coin_found)ni(n[3],n[4],n.kind)
if(n.coin_found)U[n.kind]+=1F+=1
end end function eD()N(s,"collected",12,15)end function eW()O(s,"collected",12,15)for n=1,#s do if(s[n].collected)U[s[n].kind]+=1F+=1
end end function eb()N(g,"visited",16,17)end function eZ()O(g,"visited",16,17)end function n4()menuitem(1,"reset save",function()menuitem(1,"are you sure?",function()for n=0,63do dset(n,0)end ns()n4()reset()dn()n3()return true end)return true end)end n5={}function nB()for n=0,127do n5[n]={}for e=0,127do n5[n][e]=mget(n,e)end end end function dn()for n=0,127do for e=0,127do mset(n,e,n5[n][e])end end end
__gfx__
