...
void vm::api_run()
{
    ...
    if (is_pc8c)
    {
        ...
        if (f)
        {
            ...
            if (fread(magic, 1, 4, f) == 4 && strncmp(magic, "PC8C", 4) == 0)
            {
                ...
                size_t read_bytes = fread(&m_ram, 1, rom_size, f);
                ::memcpy(&m_cart.get_rom(), &m_ram, rom_size);
                Serial.printf("[DEBUG] SD PC8C api_run load: ROM loaded (%u bytes, fread returned %u)\n", rom_size, (unsigned)read_bytes);
                Serial.printf("[DEBUG] Loaded song[0] bytes: %02X %02X %02X %02X\n",
                              m_ram.song[0].data[0], m_ram.song[0].data[1],
                              m_ram.song[0].data[2], m_ram.song[0].data[3]);
    ...
    if (game_part && !is_bios && !game_loaded)
    {
