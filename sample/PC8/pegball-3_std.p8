...
function _update60()
 ...
 elseif game_state == "power_choice" then
    if randomization_frames > 0 then
        ...
        randomization_frames = randomization_frames - (1)
    else
        if btnp(2) then
            selected_mode = 1
 ...
 elseif game_state == "classic" or game_state == "time_trial" then
	  ...
	  if active_power_up and active_power_up.name == "ball_start" and not ball.is_fired then
	      ball_start_frames = ball_start_frames + (1)
	  ...
	  if ball.is_fired then
	      if ball.special_physics == "multiball" then
	          for _i_b=1,#active_balls do local b=active_balls[_i_b]
			        ...
			        elseif active_power_up.name == "gravity_flip" then
															if ball.is_fired then
															    ...
															    elseif ball.stuck_frames >= 120 then
															        stage_complete_pending = true
															        game_state = "pause"
															    elseif (ball.y > 128 or ball.stuck_frames >= 120) and btn(5) and abs(ball.vy) < 0.1 then
															        stage_complete_pending = true
															        game_state = "pause"
															    end
															end    
 ...
 elseif game_state == "pause" then
  ...
  if pause_frames >= 90 then
      ...
      total_points_per_stage = total_points_per_stage + (points_per_shot)
      if total_points + points_per_shot < 10000 then
          total_points = total_points + (points_per_shot)
      else
          local new_total = total_points + points_per_shot
          overflow = overflow + (flr(new_total / 10000))
    ...
    if active_power_up then
				    ...
				    if active_power_up.name == "paddle" then
								...
								elseif active_power_up.name == "gravity_flip" and ball.is_fired and grav_timer > 0 then
								    print("hold 5   countdown: " .. flr(grav_timer / 60) .. " sec", 11, 120, 7)
								...
								elseif active_power_up.name == "ball_move" and not ball.is_fired then
								    print("move left 3 2 move right", 13, 120, 7)
								end
				end
...
function draw_power_choice()
    ...
    print("high risk/reward press 2", 10, 2, 7)
    draw_tiles(power_choices.high, 18, 10)

    print("medium risk/reward press 1", 10, 21, 7)
    draw_tiles(power_choices.mid, 18, 29)

    print("low risk/reward press 0", 10, 40, 7)
    draw_tiles(power_choices.low, 18, 48)

    print("overdrive press 5", 10, 59, 7)
...
function draw_how_to_1()
    ...
    print("0 aim left   aim right 1", 15, 50, 7)
    ...
    spr(56, 25, 108)
    print("press 2 for next page", 18, 120, 7)
...
function draw_how_to_2()
    ...
    local instructions = {
        ...
        {12, 103, "gravity flip, hold 5 to flip", 8},
    ...
    print("prevous page 0 1 next page", 8, 120, 7)
end
...
function draw_how_to_3()
    ...
    print("prevous page 0 5 start screen", 3, 120, 7)
end
...
function ball_fired()
  if not ball.is_fired then
	   local aim_increment = btn(4) and 0.001 or 0.008
      ...
      aim_angle = aim_angle + (aim_change)
      ...
      if btnp(0) and current_index > 1 then
          current_index = current_index - (1)
      elseif btnp(1) and current_index < #angles then
          current_index = current_index + (1)
      end
      ...
      aim_angle = angles[current_index]
	   ...
	   elseif active_power_up and active_power_up.name == "wobble_aim" then
		    ...
		    wobble_timer = wobble_timer + (0.05 )
		
		    local base_increment = 0
		    if btn(0) then
		        base_increment = base_increment - (0.008 )
		    elseif btn(1) then
		        base_increment = base_increment + (0.008 )
		    end
		    ...
		    aim_angle = aim_angle + (base_increment + wobble_effect)
      ...
      if btn(0) then
          aim_angle = aim_angle - (aim_increment)
      elseif btn(1) then
          aim_angle = aim_angle + (aim_increment)
      end
...
function handle_aiming()
  ...
  if active_power_up and active_power_up.name == "random_aim" then
    ...
    aim_angle = aim_angle + (0.03 * random_aim_direction)
    ...
    if points_per_shot < 1500 then
        ...
        for _, r in ipairs(rewards) do
            if points_per_shot >= r[1] then
                balls_remaining = balls_remaining + (r[2])
                break
            end
        end
    end
...
function reload_ball()
    ...
    if power_peg_reload_counter < 4 then
		    power_peg_reload_counter = power_peg_reload_counter + (1)
		  end
		  ...
		  if power_peg_reload_counter == 4 and power_ups_enabled then
		    if power_peg_reference and power_peg_reference.state == "dead" then
		      ...
		      for _i_peg=1,#peg_data do local peg=peg_data[_i_peg]
...
function check_valid_position(x, y)
  ...
  for _i_mirage=1,#mirage_pegs do local mirage=mirage_pegs[_i_mirage]
...
function update_time_elapsed()
    ...
    time_elapsed_frame_counter = time_elapsed_frame_counter + (1)
...
function update_peg_hit_count(peg_type)
    ...
    if peg_type == "basic" then
        basic_pegs_hit_per_shot = basic_pegs_hit_per_shot + (1)
        basic_pegs_hit_total = basic_pegs_hit_total + (1)
        points_per_shot = points_per_shot + (basic_points)
    elseif peg_type == "goal" then
        goal_pegs_hit_per_shot = goal_pegs_hit_per_shot + (1)
        goal_pegs_hit_total = goal_pegs_hit_total + (1)
        points_per_shot = points_per_shot + (goal_points)
    elseif peg_type == "bonus" then
        bonus_peg_hits_per_shot = bonus_peg_hits_per_shot + (1)
        bonus_peg_hits_total = bonus_peg_hits_total + (1)
...
function finalize_shot_points()
  if bonus_peg_hits_per_shot > 0 then
      points_per_shot = points_per_shot * (2)
  end
...
function stage_balls_earned()
  ...
  if balls_earned + balls_remaining >= 16 then
    ...
    balls_remaining = balls_remaining + (balls_earned)
  end
end
...
function total_clear()
    ...
    total_clear_sum = total_clear_sum + (1)
...
function update_physics(ball, peg_data)
  ...
  for _i_b=1,#balls_to_update do local b=balls_to_update[_i_b]
    if b.is_fired then
      b.vy = b.vy + (gravity )
      local sub_steps = 5
      ...
      for step = 1, sub_steps do
        ...
        elseif b.special_physics == "magnet" then
          ...
          if nearest_peg then
              ...
              if distance > 0 and distance < 40 then 
                  ...
                  b.vx = b.vx + (nx * attraction_strength)
                  b.vy = b.vy + (ny * attraction_strength)
              end
          end
        ...
        if active_power_up and active_power_up.name == "threepeat" then
	         ...
	         if ball.y > 126 and drops_remaining > 0 then
	      			  ball.y = 1
	      			  drops_remaining = drops_remaining - (1)
  ...
  for _i_peg=1,#peg_data do local peg=peg_data[_i_peg]
    if peg.recently_hit and peg.recently_hit > 0 then
      peg.recently_hit = peg.recently_hit - (1)
    end
  end
end
  ...
  if ball.special_physics == "multiball" then
    for _i_b=1,#active_balls do local b=active_balls[_i_b]
      ...
      elseif abs(b.x - b.last_x) < stuck_tolerance and abs(b.y - b.last_y) < stuck_tolerance then
        b.stuck_frames = b.stuck_frames + (1)
    ...
    if abs(ball.x - ball.last_x) < stuck_tolerance and abs(ball.y - ball.last_y) < stuck_tolerance then
      ball.stuck_frames = ball.stuck_frames + (1)
    else
      ball.stuck_frames = 0
      ball.last_x = ball.x
      ball.last_y = ball.y
...
function paddle()
    paddlex = mid(27, paddlex + (btn(1) and 2.2 or 0) - (btn(0) and 2.2 or 0), 101)
end
